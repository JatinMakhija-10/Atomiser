...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'f' && e.ctrlKey) { e.preventDefault(); toggleFullscreen(); }
    if (e.key === 'e' && e.ctrlKey) { e.preventDefault(); openModal('exportModal'); }
    if (e.key === 's' && e.ctrlKey) { e.preventDefault(); openModal('scheduleModal'); }
    if (e.key === ' ' && !e.target.closest('input, select, textarea')) { e.preventDefault(); toggleAtomiser(); }
    if (e.key === '?' && e.shiftKey) { document.body.classList.toggle('show-shortcuts'); }
  });

  // 3D Tilt effect on gauge cards
  initTiltCards();
...
function initMeshBackground() {
  const container = document.getElementById('meshBg');
  if (!container) return;
  ...
  function draw() {
    ...
    const isLight = document.body.classList.contains('light');
    const lineColor = isLight ? 'rgba(14, 165, 233, 0.06)' : 'rgba(56, 189, 248, 0.04)';
    const dotColor = isLight ? 'rgba(14, 165, 233, 0.12)' : 'rgba(56, 189, 248, 0.08)';
...
function measurePing() {
  ...
  fetch(`${API_BASE}/api/status`, { method: 'HEAD', cache: 'no-store' })
    .then(() => {
...
function updatePingDisplay() {
  ...
  if (lastPing === null) {
    el.textContent = '--';
    el.className = 'ping-badge bad';
    el.innerHTML = '<span class="dot"></span>--ms';
    return;
  }
  el.className = 'ping-badge' + (lastPing > 500 ? ' bad' : lastPing > 200 ? ' slow' : '');
  el.innerHTML = `<span class="dot"></span>${lastPing}ms`;
}
...
function playAlertSound() {
  ...
  try {
    ...
    osc.frequency.setValueAtTime(660, actx.currentTime + 0.1);
...
function openFullscreenChart() {
  ...
  setTimeout(() => {
    const canvas = document.getElementById('chartFullscreen');
    if (!canvas) return;
    if (!fullscreenChart) {
      fullscreenChart = new SensorChart('chartFullscreen');
    }
    fullscreenChart.resize();
    // Copy data from main chart
    if (chart && chart.data) {
      fullscreenChart.data = { ...chart.data };
      if (chart.threshold) fullscreenChart.setThreshold(chart.threshold);
